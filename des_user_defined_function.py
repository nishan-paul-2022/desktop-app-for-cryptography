...
class DES:
    ...
    def string_to_bit_array(self, text):
        arr = list()
        for char in text:
            binval = self.char_to_bit_array(char, 8)
    ...
    @staticmethod
    def char_to_bit_array(val, bitsize):
    ...
    def substitute(self, d_e):
        ...
        for i in range(len(subblocks)):
            ...
            bin = self.char_to_bit_array(val, 4)
    ...
    def run(self, key, text, action):
        ...
        for block in text_blocks:
            ...
            result += self.permut(d + g, IP_1)

        final_res = self.bit_array_to_string(result)
        return final_res


def des_user_defined_function_ende(flag, mykey):
    ...
    for path in paths:
        ...
        message = cipher.run(key, text, flag)
        message = message.split('\r\n')

        for i in range(len(message)):
            message[i] = bytes(message[i], 'unicode_escape') if flag else (message[i].strip(' ') + '\n')

        fname = generate_name(path, flag, 'des_user')
        write_file(fname, message) if flag else write_file(fname, message, 'w')
